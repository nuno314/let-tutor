...
import '../../presentation/common_widget/image_cropper/image_cropper_screen.dart';
...
class ImagePicker {
  ...
  Future<File?> show() {
    ...
    showActionDialog(
      context,
      title: title,
      titleBottomBtn: localization.cancel,
      actions: {
        localization.takePhoto: () async {
          final file = await _openCamera();
          completer.complete(file);
        },
        localization.selectPhoto: () async {
  ...
  Future<File?> _openGallery() async {
    ...
    if (!checks.any((e) => !e)) {
      ...
      final pickedFile = await PickFileHelper.pickFiles(
        dialogTitle: localization.avatar,
        type: FileType.image,
        allowMultiple: false,
      );
